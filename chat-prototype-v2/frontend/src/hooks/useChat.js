...
export function useChat(solicitudId, usuario, rol) {
  ...
  useEffect(() => {
    ...
    socket.on("receiveMessage", (mensaje) => {
      setMensajes((prev) => {
        // Evitar duplicados si el upload ya agregó el mensaje localmente
        if (prev.find((m) => m.id === mensaje.id)) return prev;
        return [...prev, mensaje];
      });
    });

    socket.on("typing", ({ usuario: quien }) => setEscribiendo(quien));
    socket.on("stopTyping", () => setEscribiendo(null));

    socket.on("usuarioUnido", () => setOtroDesconectado(false));
  ...
  const notificarEscribiendo = () => socketRef.current?.emit("typing", { solicitudId });
  const notificarDejoDeEscribir = () => socketRef.current?.emit("stopTyping", { solicitudId });
